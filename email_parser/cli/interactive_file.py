...
from .components.error_handler import ConversionErrorHandler
...
class InteractiveFileConverter:
    ...
    def __init__(self, config=None, profile_manager=None):
        self.console = Console()
        ...
        # Store shared configuration if provided
        self.shared_config = config
        self.shared_profile_manager = profile_manager
        self.parent_cli = None  # Reference to parent CLI for navigation
    ...
    async def _directory_conversion(self) -> None:
        ...
        if Confirm.ask("Proceed with batch conversion?"):
            await self._perform_conversion(
                discovery_result.convertible_files, profile, Path(output_dir)
            )
    ...
    def _display_discovery_results(self, result: FileDiscoveryResult) -> None:
        ...
        for file_type, stats in type_stats.items():
            ...
            table.add_row(f"📄 {file_type.upper()}", str(stats["count"]), f"{size_mb:.1f} MB")
        ...
        self.console.print(
            f"[dim]Estimated conversion time: {result.estimated_total_time:.1f} seconds[/dim]"
        )
...
[bold]Estimated time:[/bold] {file.estimated_conversion_time:.1f}s
    ...
    def _generate_recommendations(self, files: List[ConvertibleFile]) -> List[str]:
        ...
        if total_size > 100 * 1024 * 1024:  # > 100MB
            recommendations.append(
                "Large file set detected - consider 'Quick Conversion' profile for faster processing"
            )
        ...
        if pdf_files:
            ...
            if large_pdfs:
                recommendations.append(
                    "Large PDF files detected - 'AI Processing' profile recommended for chunking"
                )
        ...
        if docx_files:
            ...
            if complex_docx:
                recommendations.append(
                    "Complex DOCX files detected - 'Research Mode' profile for enhanced extraction"
                )

        # Batch processing recommendations
        if len(files) > 20:
            recommendations.append(
                "Large batch detected - 'Batch Optimization' profile for best throughput"
            )
    ...
    def _estimate_conversion_time(self, file_path: Path) -> float:
        ...
        base_time = {"pdf": 2.0, "docx": 1.5, "excel": 1.0}.get(file_type, 1.0)
    ...
    async def _perform_conversion(
        self, files: List[ConvertibleFile], profile: ConversionProfile, output_dir: Path
    ) -> None:
                    ...
                    try:
                        ...
                        if result["success"]:
                            ...
                            recovery_action = await self.error_handler.handle_error(
                                mock_error, error_context
                            )

                            if recovery_action:
                                retry_count += 1
                                self.console.print(
                                    f"[cyan]Retrying conversion... (attempt {retry_count}/{max_retries})[/cyan]"
                                )
                                continue
                            else:
                                failed_files.append((file.path.name, error_msg))
                                self.console.print(
                                    f"[red]Failed to convert {file.path.name}: {error_msg}[/red]"
                                )
                                break
                        ...
                        if recovery_action and retry_count < max_retries - 1:
                            ...
                            self.console.print(
                                f"[red]Failed to convert {file.path.name}: {e}[/red]"
                            )
                            break
    ...
    async def _convert_with_profile(
        self, file_path: Path, profile: ConversionProfile, output_dir: Path
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            result = await loop.run_in_executor(
                None, converter.convert_file, file_path, output_dir, None  # options
            )
    ...
    def _map_profile_to_config(
        self, profile_settings: Dict[str, Any], output_dir: str
    ) -> ProcessingConfig:
        """
        ...
        if profile_settings.get("convert_docx", False):
            ...
            if profile_settings.get("docx_chunking", False):
                config.docx_conversion.max_chunk_tokens = profile_settings.get(
                    "docx_chunk_size", 2000
                )
                config.docx_conversion.chunk_overlap = profile_settings.get(
                    "docx_chunk_overlap", 200
                )
    ...
    async def _run_quality_analysis(
        self, files: List[ConvertibleFile], output_dir: Path, profile: ConversionProfile
    ) -> None:
    ...
    def _find_converted_file(
        self, original_path: Path, output_dir: Path, file_type: str
    ) -> Optional[Path]:
        ...
        if file_type == "pdf":
            ...
            possible_names = [f"{base_name}.md", f"{base_name}_converted.md", f"{base_name}_pdf.md"]
    ...
    def _display_batch_quality_results(self, batch_analysis: Dict[str, Any]) -> None:
        ...
        if quality_by_type:
            ...
            for file_type, stats in quality_by_type.items():
                type_table.add_row(
                    file_type.upper(), f"{stats['average']:.1%}", str(stats["count"])
                )
        ...
        if common_issues:
            issues_text = "\n".join(
                f"• {issue[0]} ({issue[1]} files)" for issue in common_issues[:5]
            )
            panel = Panel(issues_text, title="🔧 Most Common Issues", border_style="yellow")
            self.console.print(panel)

    async def _show_individual_quality_reports(
        ...
        for i, result in enumerate(conversion_results):
            ...
            try:
                ...
                self.console.print(
                    f"[red]Error analyzing {result.get('input_path', 'unknown')}: {e}[/red]"
                )
                continue
