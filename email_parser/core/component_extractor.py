...
class ComponentExtractor:
        ...
        for attachment in attachment_info:
            if attachment.get("is_excel"):
                try:
                    ...
                    is_valid_excel = (file_content.startswith(xlsx_sig) or 
                                     file_content.startswith(xls_sig))
                    ...
                    logger.info(f"Converted Excel file {original_filename} to {len(conversions)} CSV files")
                    
                except Exception as e:
                    logger.error(f"Failed to convert Excel file {attachment.get('original_filename')}: {str(e)}")
            ...
            if self.processed_components.get("attachments"):
                ...
                for idx, attachment in enumerate(self.processed_components["attachments"]):
                    ...
                    if excel_conversions:
                        ...
                        for conv in excel_conversions:
                            attachment_html += f"<li>{conv['sheet_name']}: {conv['csv_filename']}</li>"
    ...
    def _find_excel_conversions(self, attachment_filename: str) -> List[Dict[str, Any]]:
        ...
        """
        return [
            conv for conv in self.processed_components.get("excel_conversions", [])
