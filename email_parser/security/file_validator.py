...
class FileValidator:
    ...
    def validate_path(self, path: str) -> None:
        ...
        if ".." in norm_path:
            raise SecurityError(
                f"Path contains path traversal sequences: {path}",
                "path_traversal"
            )
        ...
        for pattern in suspicious_patterns:
            if re.search(pattern, norm_path, re.IGNORECASE):
                raise SecurityError(
                    f"Path contains suspicious pattern: {pattern} in {path}",
                    "suspicious_path"
                )
    ...
    def _validate_file_extension(self, filename: str) -> str:
        ...
        if extension in self.blocked_extensions:
            raise SecurityError(
                f"File extension is blocked: {extension}",
                "blocked_extension"
            )
        ...
        if self.allowed_extensions is not None and extension not in self.allowed_extensions:
            raise SecurityError(
                f"File extension is not allowed: {extension}",
