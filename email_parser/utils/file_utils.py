...
def ensure_directory(directory_path: str) -> None:
    """Create directory with appropriate permissions for any OS."""
    try:
        os.makedirs(directory_path, exist_ok=True)
        # Check if directory exists after creation
        if not os.path.isdir(directory_path):
            raise SecurityError(
                f"Failed to create directory: {directory_path}", "directory_creation_failed"
            )
    except Exception as e:
        raise SecurityError(f"Directory creation error: {str(e)}", "directory_creation_failed")
